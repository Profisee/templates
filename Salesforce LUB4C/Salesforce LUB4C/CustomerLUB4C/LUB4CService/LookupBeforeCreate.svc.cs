		...
		public LUB4CResponse PerformLookup(LUB4CRequest request)
		{
			...
			try
			{
				...
				if (matchingStrategy.EntityId.Name != request.DataContext.Entity)
					response.Errors.Add(new Error { Code = "LUB4C.InvalidEntity", Description = $"Matching Strategy Entity ({matchingStrategy.EntityId.Name}) does not match Request Entity ({request.DataContext.Entity})", ErrorType = ErrorType.Error });
			...
			{
				response.Errors.Add(new Error { Code = "LUB4C.Error", Description = ex.ToString(), ErrorType = ErrorType.Error });
			}
